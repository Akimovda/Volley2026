<x-voll-layout body_class="tournament-public-page">
	...
	<div class="container">
		...
		<div class="ramka">
							...
							<table class="table">
								<thead>
									<tr>
										...
										<th style="text-align:left">{{ __('tournaments.standings_col_team') }}</th>
										<th style="text-align:center">{{ __('tournaments.standings_col_played') }}</th>
										<th style="text-align:center">{{ __('tournaments.standings_col_w') }}</th>
										<th style="text-align:center">{{ __('tournaments.standings_col_l') }}</th>
										<th style="text-align:center">{{ __('tournaments.pub_sets_col') }}</th>
										<th style="text-align:center">{{ __('tournaments.tv_diff_col') }}</th>
										<th style="text-align:center">{{ __('tournaments.tv_pts_col') }}</th>
									</tr>
								</thead>
								<tbody>
									...
									<tr>
										...
										<td style="text-align:center;color:#dc2626">{{ $s->losses }}</td>
										<td style="text-align:center">{{ $s->sets_won }}:{{ $s->sets_lost }}</td>
										<td style="text-align:center">{{ $s->pointDiff() > 0 ? '+' : '' }}{{ $s->pointDiff() }}</td>
										<td class="b-600" style="text-align:center">{{ $s->rating_points }}</td>
									</tr>
									@endforeach
								</tbody>
							</table>
			...
			<div class="card mt-2">
				<div class="b-600 cd mb-2">{{ $gName ? 'Матчи ' . $gName : 'Матчи' }}</div>
		...
		@include('tournaments._partials.participant_rating_table', ['event' => $event])
