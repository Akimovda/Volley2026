...
- **Известный нюанс (не исправлен): `tournament_individual` не заполняет tournament_teams_max/registered в meta вообще** — условие в `buildAvailabilitySnapshot` проверяет только `team_classic/team_beach/team`. Карточка при этом ВСЕГДА подписывает юнит «команд» для любого `format=tournament` (независимо от registration_mode) → для tournament_individual показывает число ЗАРЕГИСТРИРОВАННЫХ ИГРОКОВ с подписью «команд» (например «26 из 26 команд» при реальных 13 командах). Числа не врут относительно вместимости, но единица измерения вводит в заблуждение.
