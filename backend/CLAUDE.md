...
- При сохранении is_recurring мероприятия: форма показывает SweetAlert с выбором что делать с уже созданными будущими повторениями
- Параметр `future_occurrences_action`: `keep` (оставить, по умолчанию) | `cancel` (отменить старые)
- SweetAlert показывается только если `is_recurring_edit` checkbox остался checked (т.е. серия не отключается)
- Контроллер `EventManagementController::update()`: при `cancel` — отменяет все future occurrences (is_cancelled=true) ДО dispatch ExpandJob; ExpandJob создаёт новые по обновлённому расписанию
- Уведомления участникам отменённых повторений отправляются ПОСЛЕ dispatch — через `notifyUsersAboutCancelledEvent(reason: 'Расписание серии изменено организатором')`
- Причина проблемы на проде: ExpandJob добавлял новые occurrence по вторникам, но старые средовые (не соответствующие новому BYDAY=TU) не отменялись — теперь решается через этот диалог
