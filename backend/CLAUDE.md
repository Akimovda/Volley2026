...
## Деплой (dev к prod)
- Прод находится на ветке `production` (НЕ `main`) — `git branch` покажет `* production`
- Деплой: merge origin/main в production; проверить наличие коммита: `git merge-base --is-ancestor <hash> HEAD && echo "уже в проде"`
...
- Partials: `resources/views/tournaments/_partials/` (создана в этой сессии)
- `group_crosstable.blade.php` — кросс-таблица (шахматка) для группы
  - Принимает: `$group`, `$groupMatches`, `$groupClean`, `$groupOutsiders`
  - Ячейки: победа=зелёный, поражение=красный, диагональ=×, не сыгран=серый
  - Итоги: М, В, Сеты, Очки, Мячи (clean stats), Место (I/II/III/... с медальными цветами)
- Переключатель Список / Шахматка: кнопки `.ct-view-btn`, блоки `.ct-view-list` и `.ct-view-crosstable` с `data-group="{id}"`
  - localStorage ключ `ct_view_pref` сохраняет выбор
  - Показывается только если у группы есть standings (`$hasCrosstable`)
- Структура данных в matches-цикле: `$stage->matches->groupBy('group_id')` → `$groupId => $groupMatches`
  - `$groupForCross = $stage->groups->firstWhere('id', $groupId)` для получения объекта группы
  - `$crossClean = $cleanStatsByGroup[$groupId] ?? []`, `$crossOutsiders = $outsidersByGroup[$groupId] ?? []`
...
- **Индивидуальная регистрация (НЕ турниры)** — `autoBookNext`:
  ...
  - Лимит: 20 итераций на один вызов (счётчик всех итераций включая continue), при превышении — warning в лог
  - Проверка: subscribedToPosition + EventRegistrationGuard::checkEligibility (возраст, уровень, окно, **гендер**)
  - checkEligibility загружает регистрации и вызывает applyGenderPolicy — не запишет неподходящий пол
  - Платные мероприятия: PaymentService::createForRegistration с expires_at — игрок должен оплатить за payment_hold_minutes (15 мин), иначе PaymentService::releaseExpired откатит и запустит следующего из очереди
  - **ВАЖНО**: releaseExpired использует Eloquent save() (не Query Builder update()) — иначе Observer не срабатывает и onSpotFreed не вызывается
...
- **Турниры (format=tournament)** — старая логика (`notifyNext` + CheckWaitlistNotificationJob):
  ...
  - Записаться в waitlist на турнир через UI нельзя (OccurrenceWaitlistController блокирует) — логика фактически не используется
- **EventRegistrationGuard::check()** — `checkWaitlistGate`:
  - Если есть кто-то в waitlist (помимо текущего user) → запись полностью закрыта (включая reserve)
  - reserve убирается из freePositions — нельзя занять запасное место в обход очереди
  - Сообщение: «Запись закрыта — доступна только запись в лист ожидания»
...
- **Слоты позиций**: классика использует event_role_slots (setter/outside/middle/opposite/libero/reserve); пляжка имеет один слот role='player'; calculatePositions строит freePositions из этих слотов
- **Взаимное исключение состав/резерв**:
  - Нельзя встать в waitlist если уже записан в состав (включая запасные места) — проверка в OccurrenceWaitlistController::store()
  - Нельзя записаться в состав если уже в waitlist — проверка в EventRegistrationController::storeOccurrence()
  - UI: блок waitlist скрывается при `$isRegistered=true`; кнопки записи скрываются при `$myWaitlist!=null`
  - `$myWaitlist` вычисляется один раз в начале @else (event not started), далее используется в обоих блоках
- **UX блока waitlist** (players.blade.php):
  - Блок показывается при `$isFull` ИЛИ при наличии занятых классических позиций (`$occupiedPositions`), И `!$isRegistered`
  - Занятые позиции классики отображаются как disabled-строки с якорной ссылкой на блок waitlist
  - Чекбоксы в форме: только занятые позиции (не свободные — нет смысла ждать доступное)
  - Пляжка: чекбоксов нет, текст подчёркивает автозапись ("запишем автоматически")
  - Классика isFull: текст "уведомим первым"; классика partial: "позиция занята"; пляжка: "запишем автоматически"
  - `getSlots` кешируется — два вызова в blade (кнопки + waitlist форма) = один SQL запрос
...
- Таблицы: `user_teams` (id, user_id, name, direction, subtype) + `user_team_members` (user_team_id, user_id, role_code, position_code)
- Модели: `UserTeam`, `UserTeamMember`
- Сервис: `UserTeamValidationService` — validateForEvent() (возраст/уровень/пол/профиль каждого участника) + checkTeamSize() (vs team_size_min + reserve_players_max)
- Контроллер: `UserTeamController` CRUD на `/user/teams/{team}`
- `TournamentTeamController::saveToProfile()` — копирует EventTeam → UserTeam (только капитан)
- `TournamentTeamController::fromSaved()` — создаёт EventTeam из UserTeam, рассылает invites (участники подтверждают сами)
- При ошибках валидации → редирект на `/user/teams/{team}/edit?event_id=X` с session('team_validation_errors')
- UI: профиль показывает «Мои команды»; players.blade.php показывает select сохранённых команд перед формой создания новой

## Позиция reserve в регистрациях на мероприятие
...
- Перевод: `__('events.positions.reserve')` = 'Запасной' / 'Reserve'
...
- Поле timezone убрано из UI, заменено на `<input type="hidden" name="timezone" id="mgmt_timezone_hidden">`
- City autocomplete → AJAX `/ajax/cities/meta` обновляет timezone + `/ajax/locations/by-city` фильтрует локации
- Контроллер грузит `location.city`, фильтрует локации по городу, передаёт `$currentCity` в view
...
### Уровень 1 — Базовый (доступ к сайту)
- `User::isProfileComplete()` проверяет только: `first_name`, `last_name`, `phone`
- `profile_completed_at` ставится когда эти 3 поля заполнены
- `patronymic`, `birth_date`, `city_id` — больше НЕ обязательны для входа на сайт
- Middleware `EnsureProfileCompleted` использует `isProfileComplete()`

### Уровень 2 — Контекстный (при записи на мероприятие)
- `User::getMissingFieldsForEvent(Event $event, ?EventOccurrence $occurrence = null): array`
- Проверяет в зависимости от настроек события: age_policy→birth_date, direction+levels→classic_level/beach_level, gender_policy→gender, requires_personal_data→все
- `EventRegistrationController::storeOccurrence` вызывает его ДО Guard
- Если есть missing → редирект на `/profile/complete?missing=...&return_to=...`
- После сохранения → редирект обратно на return_to

### Блокировка полей (protectedOnce)
- Заблокированы после заполнения (для обычных пользователей): `first_name`, `last_name`, `patronymic`, `phone`, `city_id`, `birth_date`, `gender`, `classic_level`, `beach_level`
- `ProfileUpdateGuard::selfEdit()` — содержит список
- `ProfileExtraController` — дублирует список (имплементация отдельная)
- Организаторы и администраторы — без ограничений на свой профиль

### OAuth при создании нового пользователя
- VK, Google, Telegram, Apple, Yandex — НЕ заполняют first_name/last_name/phone/gender при создании
- Сохраняется только: name (отображаемое), email, provider_id, password (random)
- Пользователь заполнит поля сам через profile/complete

### profile/complete.blade.php
- `$missingKeys` — поля незаполненные для конкретного мероприятия (из getMissingFieldsForEvent)
- `$returnTo` — URL возврата после сохранения (event show page)
- JS-подсветка полей: добавляет class `input-error` на соответствующие элементы и скроллит к первому
- Форма постит на `route('profile.extra.update')` (ProfileExtraController), не на ProfileCompletionController
...
- Уникальные OAuth поля: `telegram_id`, `vk_id`, `yandex_id`, `apple_id`, `google_id` — берутся из secondary только если у primary пусто; сначала обнуляются у secondary (unique constraint)
- Не-уникальные поля провайдеров: `telegram_username`, `yandex_phone`, `telegram_phone`, `vk_phone`
...
- `classic_level` / `beach_level` — остаются от primary если заполнены; из secondary берутся только если primary пуст
- `user_beach_zones` — переносятся с дедупликацией по `zone` (шаг 16)
- `user_classic_positions` — переносятся с дедупликацией по `position` (шаг 17)
...
- НЕ переносятся (не критично): event_team_member_audits, notification_deliveries, page_views, account_delete_requests
