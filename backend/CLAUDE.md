...
## Мониторинг и бэкапы (backup-сервер, отдельная машина — 185.225.232.210, не прод)
Владелец backup-сервера и обоих сервисов ниже — mobile-репозиторий (отдельная Claude-сессия), не эта сессия. Если что-то из перечисленного не отвечает/сломано именно на СТОРОНЕ backup-сервера — не чинить самостоятельно, сообщить пользователю.
- **GlitchTip** (error tracking, Sentry-совместимый) — `https://errors.akimoff.su`, org `dmitry`, проект `volleyplay-backend`. API: `Authorization: Bearer <token>` (токен — `GLITCHTIP_API_TOKEN` в `.env`, НЕ хранить в этом файле), базовый путь `/api/0/...`. `GET /api/0/projects/dmitry/volleyplay-backend/issues/` — список unresolved ошибок; `GET /api/0/issues/{id}/` — детали; `POST /api/0/issues/{id}/` `{"status":"resolved"}` — закрыть; `GET /api/0/projects/dmitry/volleyplay-backend/events/{event_id}/` — стектрейс события. Алерты в Telegram (@VolleyClub_Alert_bot) уже настроены. **При расследовании прод-багов — сначала issues в GlitchTip, обычно быстрее чем логи Laravel вручную.**
- **Backend SDK (Laravel → GlitchTip), подключено 2026-09-25**: пакет `sentry/sentry-laravel`, конфиг `config/sentry.php`, интеграция — `Sentry\Laravel\Integration::handles($exceptions)` в `bootstrap/app.php` (`withExceptions`). DSN — `SENTRY_LARAVEL_DSN` в `.env` каждой среды отдельно (не коммитится, не дефолтится). **`environment` НЕ равен `APP_ENV`** — на этом сервере DEV и PROD оба имеют `APP_ENV=production` в `.env` (инфраструктурная особенность, отдельная задача, не трогали) — вместо этого отдельная `SENTRY_ENVIRONMENT` (dev/production) в `.env`, если не задана — SDK сам подставит `app()->environment()` (см. `Sentry\Laravel\ServiceProvider::bindEvents()`). `release` — короткий git sha текущего HEAD (вычисляется при `config:cache`; репозиторий на уровень выше document root — `dirname(base_path())/.git`, т.е. `/var/www/volley-bot/.git` и `/var/www/volleyplay/.git`, НЕ `base_path('.git')`). `traces_sample_rate=0` и `send_default_pii=false` — жёстко в конфиге, не через env (только error tracking, никакого perf-трейсинга и PII). `send_default_pii=false` отключает у пакета встроенный сбор пользователя целиком (id/email/username, см. `ServiceProvider::bindEvents()` — `subscribeAuthEvents` регистрируется только если `send_default_pii!==false`) — вместо этого `App\Http\Middleware\SentryUserContext` (глобальный middleware, `bootstrap/app.php`) кладёт в scope только `user.id`. `ValidationException`/`AuthenticationException`/`NotFoundHttpException` (404) исключены явным `$exceptions->dontReport([...])` (дублирует `$internalDontReport` самого Laravel — не полагаемся на недокументированный дефолт фреймворка). После любого деплоя с изменением `.env`/`config/sentry.php`/`bootstrap/app.php` — `config:cache` + `reload php8.3-fpm` (иначе новый DSN/код не подхватится, тот же паттерн, что и везде в этом файле).
- **Uptime Kuma v2** (аптайм) — `https://status.akimoff.su`. Авторизация — HTTP Basic Auth, логин пустой, пароль — API-ключ (`UPTIME_KUMA_API_KEY` в `.env`): `curl -u ":<key>" https://status.akimoff.su/metrics`. Ответ — Prometheus text format (`monitor_uptime_ratio`, `monitor_response_time_seconds`, `monitor_cert_days_remaining` и т.п.). REST API для управления мониторами НЕТ (только Socket.IO из веб-UI) — правки конфигурации мониторов делает пользователь сам через UI. Сейчас мониторится volleyplay.club (monitor_id=1), алерты в тот же Telegram-бот. **Перед тем как чинить «сайт не работает» — сначала `/metrics`, чтобы понять с какого момента и что именно упало.**
- **Система бэкапов** (off-server, тот же backup-сервер, настроена и проверена сентябрь 2026 — НЕ моя зона настройки): БД `pg_dump -Fc` ежедневно 02:00 (retention 30д, ежемесячный restore-тест); `.env`+ключи интеграций (APNs/Apple Sign In/FCM) ежедневно 02:10 (retention 30д); `storage/app/public` ежедневно 02:20 (зеркало); конфиг сервера (nginx/php-fpm/supervisor/systemd/cron/версии пакетов) еженедельно вс 01:45 (`/opt/volleyplay-ops/collect_server_config.sh` на проде); ежедневная Telegram-сводка 02:35 подтверждает доставку — отсутствие сводки несколько дней подряд = тревожный знак сам по себе. Чек-лист восстановления — `report/disaster-recovery-checklist.md` в мобильном репозитории (не здесь). Если на проде появляется что-то, что должно бэкапиться, но не бэкапится (новый .env-подобный файл, новая директория с важными данными) — сказать пользователю, не молчать и не чинить самостоятельно (пайплайн настраивается с другой машины).
