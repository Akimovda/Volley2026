...
- **Rate-limit (1/сутки на пользователя)**: `NotifyCityAboutNewEventJob` перед вызовом `create()` батчем проверяет `user_notifications` (`type='new_event_in_city'`, `created_at >= now()-24ч`) для всех ID чанка разом (`whereIn`, не по одному) и пропускает уже уведомлённых. Организатор создал 3 события в городе за день → жители получат уведомление только про первое, остальные два скипаются на этапе rate-limit (проверено на dev: `before=2 after=2` после второго события).
...
- **Тестирование**: полностью синтетические данные в городе БЕЗ реальных пользователей (Воронеж, city_id=128, был найден именно как город с 0 реальных юзеров + 1 локацией) — организатор + 3 резидента (с Telegram/без каналов/opted-out), `telegram_notify_chat_id` резидента-теста указывал на тот же админ-чат "AdminChatVolleyPlay", что и в тесте финализации анонсов (см. выше) — реальная доставка подтверждена (`SendNotificationDeliveryJob` DONE, `notification_deliveries.status=sent`). Всё удалено после теста.

### Побочная находка при этой сессии: `.checkbox-item`/`.custom-checkbox` требуют предка с классом `.form`
- ВСЕ правила в `style.css` для чекбоксов/радио скопом `.form .checkbox-item`/`.form .custom-checkbox` (включая `.form .checkbox-item input {display:none}` — без этого правила нативный чекбокс не скрывается, кастомный скин рисует ПУСТОЙ `<div>` без размеров рядом). Без предка `.form` где-либо выше по DOM — виден голый браузерный чекбокс, кастомный тумблер не работает вообще (не «немного не так выглядит», а полностью отсутствует).
- Аудит всех 32 файлов с `checkbox-item` на сайте (2026-07-17) — **ложных срабатываний оказалось больше, чем реальных проблем**: первый грубый grep («есть ли `class="form"` где-то в файле») давал много false positive/negative — реальная проверка требует смотреть на класс как ОТДЕЛЬНЫЙ ТОКЕН (`"mt-2 pt-2 form"` матчится, `"form-control"` — нет), и учитывать что `.form` часто находится в РОДИТЕЛЬСКОМ файле, который `@include`-ит партиал (`events/create.blade.php` — один `<div class="form">` в начале обёртывает ВСЕ степ-партиалы `step1/step2/step3/payment/personal_data/registration/reminder`; `tournaments/setup.blade.php` — аналогично, один `<div class="container form">` в начале страницы покрывает ВСЮ страницу включая `@include('tournaments._partials.king_beach_stage', ...)`). После точной проверки (per-token regex + проверка родителей) реальных пропусков `.form`-скоупа на сайте НЕ нашлось — только 4 места использовали НАТИВНЫЙ чекбокс без обёртки `checkbox-item`/`custom-checkbox` ВООБЩЕ (но внутри уже существующего `.form`-скоупа, так что фикс был чисто добавлением markup, без риска сломать стилизацию): `admin/locations/edit.blade.php` (`direction-toggle`, `court_indoor`, `day-off-toggle` — все внутри `<form class="form" id="directionsForm">`), `admin/users/show.blade.php` (`is_club_manager`, внутри `<form class="form">`), `profile/widget.blade.php` (`show_slots`/`show_location`, внутри `<form class="form">`), `tournaments/_partials/king_beach_stage.blade.php` (`kb-player-cb` bulk-select — внутри унаследованного `.form` от `setup.blade.php`). Везде JS-селекторы (`.direction-toggle`, `.day-off-toggle`, `.kb-player-cb`) читают класс на самом `<input>` — обёртка в `<label class="checkbox-item">` + добавление `<div class="custom-checkbox"></div>` не меняет DOM-класс инпута, JS не сломан.
- **Осознанно НЕ трогали**: `resources/views/components/checkbox.blade.php` (`<x-checkbox>`, Jetstream-компонент, Tailwind-классы) — используется в `register.blade.php` (согласие с условиями) и `api-token-manager.blade.php` (Livewire `wire:model`) — отдельная дизайн-система, не относится к кастомному `.checkbox-item`.
...
- Исправлено 16 мест: `locations/show.blade.php`, `welcome.blade.php` (тултип на карте), `locations/index.blade.php` (×2), `user/public.blade.php`, `volleyball_school/show.blade.php`, `admin/locations/index.blade.php`, `admin/locations/edit.blade.php`, `event_management_edit.blade.php` (PHP-префилл + JS `renderResults`), `events/_partials/create/step2.blade.php`, `profile/complete.blade.php`, `profile/show.blade.php`, `users/show.blade.php`, `users/_card.blade.php`, `users/index.blade.php`, `public/assets/city.js`, `public/js/events-create.js`.
- **Осознанно НЕ трогал**: `welcome.blade.php` строка с `$isoCodes[$city->region]` — это функциональный ключ для флага страны (не отображение), менять нельзя.
- Для обычных городов (Новосибирск/Новосибирская область) `region_display` возвращает регион как есть — проверено на реальных данных БД и на реальном ответе `/cities/search`: у самого СПб (id=266) `region_display=null`, у его районов (Калининский и т.д., у которых `region="Санкт-Петербург"`, но `name` другое) `region_display` заполнен корректно.
...
- **Тестирование на dev**: синтетические Event/EventOccurrence + `UserNotificationChannel` с `chat_id = config('services.telegram.admin_chat_id')` (существующий админ-чат "AdminChatVolleyPlay", НЕ реальный организаторский канал с живыми подписчиками — на dev каналы из `user_notification_channels` в основном привязаны к настоящим боевым каналам организаторов). После теста — DB-запись удалена, тестовое Telegram-сообщение удалено через `TelegramChannelPublisher::delete()`.
