...
- **Выход с добавлением в waitlist**: `leaveTeam` контроллера принимает `add_to_waitlist` (bool). Позицию участника читать ДО удаления (`$member->position_code`), затем `WaitlistService::join($occurrence, $user, $positions)`. Если join бросает исключение — ловить и добавлять в success-сообщение, не прерывать выход.
