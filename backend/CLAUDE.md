...
### Таблицы
- `activity_sessions` — одна запись на тренировку: avg/max/min/hr, duration_sec, load_score, **calories_kcal** (decimal 7,2, null), time_in_zone (jsonb), samples_count
- `activity_hr_samples` — сырые сэмплы (session_id, t_offset_sec, bpm); uq constraint → idempotent ingest
- `athlete_devices` — зарегистрированные BLE-устройства пользователя
- `athlete_profiles` — resting_hr, max_hr, weight_kg (нужен для калорий)
- **`user_consents`** — append-only согласия: user_id, type ('health_activity'), document_version, locale, accepted_at; index (user_id, type)

### Конфиг
- `config/activity.php`: `recording_open` (env ACTIVITY_RECORDING_OPEN, по умолч. false) + `consent_version` (env ACTIVITY_CONSENT_VERSION, по умолч. '2026-06-21')
- Запись открыта только для админов пока `recording_open=false`
...
- `POST /api/activity/sessions/{id}/finalize` — завершить (возвращает avg/max/min/load/calories/zones)
