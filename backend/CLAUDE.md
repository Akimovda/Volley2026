...
- **НЕ пофикшено (осознанно, вне скоупа)**: сам дрейф `event_occurrence_stats` в `EventRegistrationsManagementController::cancel()` (кеш по-прежнему может быть неверным для других потребителей); превышение 100% загрузки, когда фактических регистраций больше `max_players` за счёт резерва (`reserve_players_max` не учтён в знаменателе).
