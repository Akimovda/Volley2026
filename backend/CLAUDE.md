...
### UI — где отображается CR
- `/players/rating` — колонка «Рейтинг» (CR), сортировка по CR по умолчанию; μ показывается мелко рядом
- Публичный профиль игрока (`user/public.blade.php`) — CR рядом с WinRate (показывается при total_matches ≥ 3)
- Страница сезона (`seasons/show.blade.php`) — колонка CR, сортировка по `mu_season - 3*sigma_season`

### i18n-ключи рейтинга (tournaments.*)
- `conservative_rating` — «Рейтинг» / «Rating»
- `mu` — «Уровень (μ)» / «Skill (μ)»
- `sigma` — «Точность (σ)» / «Uncertainty (σ)»
- `rating_info` — подсказка про точность
